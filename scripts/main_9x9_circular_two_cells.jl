...
"""
Test script for RSP (Random Spread Process) with 9x9 grid and 4 agents with circular trajectories.
Agents choose a SUBSET OF TWO CONTIGUOUS CELLS in the Field of Regard (FOR) per step.
Uses MAX_SENSING_TARGETS=2 and CONTIGUOUS_PAIRS_ONLY; does not affect main.jl / main_5x5_complex.jl.

Planning mode `:pbvi_rollout` uses `src/planners/macro_planner_pbvi_rollout.jl` (seed-averaged rollouts over
sampled macro-sequences). Tune `PBVI_ROLLOUT_N_SEQUENCES` and `set_pbvi_rollout_n_sequences_from_main` below;
`PBVI_N_SEED` controls rollouts per candidate via `set_hyperparams_from_main`.
"""

println("🚀 RSP 9x9 Circular Trajectory Test starting...")
...
#const modes = [:oracle, :prior_based, :random, :greedy, :pbvi_1_0_0_0, :pbvi_0_5_0_5, :pbvi_0_0_1_0, :pomcp]  # Test only the policy tree mode
# :pbvi_rollout — seed-averaged open-loop scoring over sampled macro-sequences (see src/planners/macro_planner_pbvi_rollout.jl);
#   uses PBVI N_seed for rollouts per candidate and PBVI_ROLLOUT_N_SEQUENCES (set via GroundStation setter below).
const modes = [:pbvi_rollout, :klolop, :pbvi_1_0_0_0, :pbvi_0_5_0_5, :pbvi_0_0_1_0, :random, :oracle, :greedy, :prior_based]
const N_RUNS = 200 
...
const POMCP_N_SIMS       = PBVI_N_SEED * PBVI_N_PARTICLES * PBVI_N_SWEEPS
const MPOMDP_N_SEQUENCES = PBVI_N_SEED * PBVI_N_PARTICLES
# Candidate macro-sequences per `:pbvi_rollout` replan (`-1` = enumerate all if product is small enough)
const PBVI_ROLLOUT_N_SEQUENCES = 1000
const KLOLOP_BUDGET      = PBVI_N_SEED * PBVI_N_PARTICLES  # total rollout steps (≈ SB-ABBA particles×seeds per sync)

# 🌍 ENVIRONMENT PARAMETERS
const GRID_WIDTH = 9                  # Grid width (columns) - CHANGED FROM 5 TO 9
const GRID_HEIGHT = 9                 # Grid height (rows) - CHANGED FROM 5 TO 9
const INITIAL_EVENTS = 1              # Number of initial events
const MAX_SENSING_TARGETS = 2         # Two contiguous cells per step (this script only)
...
# 🤖 AGENT PARAMETERS
const NUM_AGENTS = 4                  # Number of agents - CHANGED FROM 2 TO 4
...
const GROUND_STATION_X = 5            # Ground station X position (center of 9x9) - CHANGED FROM 2 TO 5
const GROUND_STATION_Y = 5            # Ground station Y position (center of 9x9) - CHANGED FROM 1 TO 5
...
# Configure POMCP, MPOMDP, KL-OLOP, and PBVI-rollout (`macro_planner_pbvi_rollout.jl` — `planning_mode=:pbvi_rollout` in GroundStation.maybe_sync!)
using .MyProject.Planners.GroundStation: set_pomcp_n_sims_from_main, set_mpomdp_n_sequences_from_main, set_pbvi_rollout_n_sequences_from_main, set_klolop_budget_from_main
...
# Two-cell contiguous FOR for this script only (main.jl and main_5x5_complex.jl keep default single-cell)
using .MyProject.Types
Types.CONTIGUOUS_PAIRS_ONLY[] = true
...
# PBVI rollout (`src/planners/macro_planner_pbvi_rollout.jl`); `GroundStation.maybe_sync!` dispatches when `planning_mode == :pbvi_rollout`
using .Planners: MacroPlannerPBVIRollout
...
# =============================================================================
# 9-CELL FIELD OF REGARD FUNCTIONS (AGENT + 8 NEIGHBORS)
# =============================================================================

"""
Get field of regard for an agent at a specific position (9-cell: agent + 8 neighbors)
FOR includes the agent's current position plus all 8 adjacent cells (including diagonals)
"""
...
function debug_agent_positions(agents)
    ...
    for agent in agents
        ...
        for t in 0:11  # Show one complete period (12 steps: 0-11)
...
"""
Create agents with circular trajectories and 9-cell sensors
"""
function create_circular_agents()
    agents = Agent[]
    
    # Design 4 circular trajectories with central overlap on a 9x9 grid
    # All trajectories must pass through or near the ground station at (5,5)
    
    # Agent 1: Upper-left circular trajectory
    # Center at (3,3), radius 2.8, period 12 - passes through center area
    trajectory1 = create_circular_trajectory(3.0, 4.0, 2.0, 12)
    
    # Agent 2: Upper-right circular trajectory  
    # Center at (7,3), radius 2.8, period 12 - passes through center area
    trajectory2 = create_circular_trajectory(7.0, 4.0, 2.0, 12)
    
    # Agent 3: Lower-left circular trajectory
    # Center at (3,7), radius 2.8, period 12 - passes through center area
    trajectory3 = create_circular_trajectory(3.0, 6.0, 2.0, 12)
    
    # Agent 4: Lower-right circular trajectory
    # Center at (7,7), radius 2.8, period 12 - passes through center area
    trajectory4 = create_circular_trajectory(7.0, 6.0, 2.0, 12)
    
    # 9-cell sensors for all agents (agent position + 8 neighbors)
    sensor1 = RangeLimitedSensor(1.5, pi/2, SENSOR_NOISE, :circular)  # Range 1.5 covers 9 cells
    ...
    # Create agents with different phase offsets to match image positions
    # With period 12, distribute agents evenly around their circles
    agent1 = Agent(1, trajectory1, sensor1, 0, MAX_BATTERY, CHARGING_RATE, OBSERVATION_COST)   # Start at 0
    agent2 = Agent(2, trajectory2, sensor2, 3, MAX_BATTERY, CHARGING_RATE, OBSERVATION_COST)   # Phase offset of 3 (1/4 circle)
    agent3 = Agent(3, trajectory3, sensor3, 6, MAX_BATTERY, CHARGING_RATE, OBSERVATION_COST)   # Phase offset of 6 (1/2 circle)
    agent4 = Agent(4, trajectory4, sensor4, 9, MAX_BATTERY, CHARGING_RATE, OBSERVATION_COST)   # Phase offset of 9 (3/4 circle)
    ...
    println("🤖 Created $(NUM_AGENTS) agents with circular trajectories and 9-cell sensors:")
    for agent in agents
        println("  Agent $(agent.id): circular trajectory with period $(agent.trajectory.period), phase offset $(agent.phase_offset)")
        println("    Sensor: RangeLimitedSensor with circular pattern (9-cell visibility: agent + 8 neighbors)")
        println("    Starting position: $(get_position_at_time(agent.trajectory, 0))")
        println("    Circle center: ($(agent.trajectory.center_x), $(agent.trajectory.center_y)), radius: $(agent.trajectory.radius)")
    end
...
function visualize_rsp_state(
    ...
    agent_colors = [:red, :blue, :green, :orange]  # 4 different colors for 4 agents

    # Create a blank plot with correct limits and aspect
    p = plot(; xlim=(0.5, width+0.5), ylim=(0.5, height+0.5),
        aspect_ratio=:equal, size=(800, 800), legend=false,  # Larger size for 9x9 grid
        xlabel="X Coordinate", ylabel="Y Coordinate",
        grid=false,
        title="RSP $(width)x$(height) Circular Trajectories - Time Step $(time_step) - γ=$(DISCOUNT_FACTOR), Events: $(count(==(EVENT_PRESENT), environment_state)) | Agents: $(length(agents)) | Detected: $(events_detected) | Avg Uncertainty: $(round(avg_uncertainty, digits=3))",
        titlefontsize=12,
    ...
    for (i, agent) in enumerate(agents)
        ...
        # Get FOR cells (9-cell pattern)
        for_cells = get_nine_cell_field_of_regard(agent, pos, (width=width, height=height))
    ...
    # Overlay: Agents as small circles with trajectory paths
    for (i, agent) in enumerate(agents)
        pos = get_position_at_time(agent.trajectory, time_step, agent.phase_offset)
        agent_color = agent_colors[i]
        scatter!(p, [pos[1]], [pos[2]]; marker=:circle, markersize=10, color=agent_color, alpha=0.9, label=false)
        
        # Draw circular trajectory path (optional - can be commented out if too cluttered)
        # circle_x = []
        # circle_y = []
        # for angle in 0:0.1:(2π)
        #     cx = agent.trajectory.center_x + agent.trajectory.radius * cos(angle)
        #     cy = agent.trajectory.center_y + agent.trajectory.radius * sin(angle)
        #     if 1 <= cx <= width && 1 <= cy <= height
        #         push!(circle_x, cx)
        #         push!(circle_y, cy)
        #     end
        # end
        # if !isempty(circle_x)
        #     plot!(p, circle_x, circle_y, color=agent_color, alpha=0.3, linewidth=1, label=false)
        # end
    end
...
function create_rsp_animation(
    ...
    anim = @animate for frame in frames
        plot(frame, size=(800, 800))  # Larger size for 9x9 grid
    end
...
function create_belief_event_animation(
    ...
    anim = @animate for (t, prob_map) in enumerate(belief_event_present_evolution)
        ...
        heatmap(
            ...
            c = :viridis,
            clims = (0.0, 1.0),
            size = (800, 800)  # Larger size for 9x9 grid
        )
    end
...
function save_performance_metrics(gs_state, avg_uncertainty, event_observation_percentage, ndd_expected_lifetime, ndd_actual_lifetime, env, agents, results_dir, run_number, planning_mode)
    ...
    open(filepath, "w") do file
        ...
        for (i, agent) in enumerate(agents)
            ...
            println(file, "    Circle center: ($(agent.trajectory.center_x), $(agent.trajectory.center_y)), radius: $(agent.trajectory.radius)")
        end
        println(file, "")
...
function simulate_rsp_async_planning_replay(replay_env::ReplayEnvironment, num_steps::Int=NUM_STEPS, run_number::Int=1, planning_mode::Symbol=:script)
    ...
    println("====================================================================")
    println("Grid: $(GRID_WIDTH)x$(GRID_HEIGHT)")
    println("Agents: $(NUM_AGENTS) (circular trajectories)")
    ...
    # Set env.current_state for oracle planner access
    ...
    return gs_state, agents, event_observation_percentage, sync_events, environment_evolution, action_history, event_tracker, uncertainty_evolution, average_uncertainty_per_timestep, ndd_life, belief_event_present_evolution, events_detected_per_timestep, reward_log_rows, cumulative_discounted_team
end
...
for n in 1:N_RUNS
    ...
    for mode in modes
        ...
        # Rollout scoring calls `GroundStation.MacroPlannerPBVI.simulate_one_step` — keep globals aligned with this script
        # after any parameterized `:pbvi` mode in the same `modes` vector.
        if actual_planning_mode == :pbvi_rollout
            set_reward_config_from_main(ENTROPY_WEIGHT, VALUE_WEIGHT, INFORMATION_STATES, STATE_VALUES)
        end
        
        println("🎯 RSP 9x9 Circular Trajectory Test")
        println("==================================")
        ...
        elseif actual_planning_mode == :pbvi_rollout
            println("  PBVI-rollout: PBVI_ROLLOUT_N_SEQUENCES=$(PBVI_ROLLOUT_N_SEQUENCES) (macro-sequence candidates), N_seed=$(PBVI_N_SEED) (rollouts per candidate)")
        end
        println("  Dynamics: Heterogeneous RSP (Replay)")
        println("  9-cell sensor: true (agent + 8 neighbors)")
        ...
        gs_state, agents, percentage, sync_events, env_evolution, action_history, event_tracker, uncertainty_evolution, uncertainty_avg, ndd_life, belief_event_present_evolution, events_detected_per_timestep, reward_log_rows, cumulative_discounted_team = simulate_rsp_async_planning_replay(replay_env, NUM_STEPS, n, actual_planning_mode)
        ...
        Types.save_event_evolution_replay_csv(replay_env.event_evolution, results_base_dir, n, PLANNING_MODE)
        Types.save_rsp_transition_probabilities_csv(replay_env.prob_no_event_evolution, replay_env.prob_event_evolution, results_base_dir, n, PLANNING_MODE)
        Types.save_planner_environment_evolution_csv(env_evolution, results_base_dir, n, PLANNING_MODE)
        Types.save_action_reward_log_csv(reward_log_rows, results_base_dir, n, PLANNING_MODE, replay_env.env.discount, cumulative_discounted_team;
        ...
        anim = create_rsp_animation(agents, NUM_STEPS, env_evolution, action_history, (GROUND_STATION_X, GROUND_STATION_Y), results_base_dir, n, PLANNING_MODE; 
                                  events_detected_per_timestep=events_detected_per_timestep, avg_uncertainty_per_timestep=uncertainty_avg)
        ...
        println("  - Run $(n)/$(PLANNING_MODE)/metrics/ (performance metrics, agent actions, NDD, ground truth + RSP kernels, reward log)")
