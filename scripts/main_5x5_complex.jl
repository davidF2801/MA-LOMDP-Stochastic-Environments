...
# Add :pbvi_rollout for seed-averaged SB-ABBA-style enumeration (uses PBVI hyperparams N_seed only)
#const modes = [:klolop,:random, :oracle,:greedy, :pbvi_1_0_0_0, :pbvi_0_5_0_5, :pbvi_0_0_1_0, :pbvi_rollout, :prior_based]  # Test only the policy tree mode
#const modes = [:klolop, :posts]
const modes = [:klolop, :posts, :pbvi_1_0_0_0, :pbvi_0_5_0_5, :pbvi_0_0_1_0, :pbvi_rollout, :random, :oracle, :greedy,:prior_based]
...
const POMCP_N_SIMS       = 1500
const MPOMDP_N_SEQUENCES = 100
const PBVI_ROLLOUT_N_SEQUENCES = PBVI_N_SEED * PBVI_N_PARTICLES  # sampled macro-sequence candidates per `:pbvi_rollout` replan (`-1` = full enumeration)
const KLOLOP_BUDGET      = PBVI_N_SWEEPS * PBVI_N_PARTICLES  # total rollout steps (≈ SB-ABBA particles×seeds per sync)
const POSTS_BUDGET       = PBVI_N_PARTICLES * PBVI_N_SWEEPS  # POSTS simulations (more rollouts = better plans)
...
const STATE_VALUES = [0.0, 1.0]      # F_1: No event value, F_2: Event value
...
function visualize_rsp_state(
    ...
    p = plot(; xlim=(0.5, width+0.5), ylim=(0.5, height+0.5),
        ...
        grid=false,
        title="RSP $(width)x$(height) Complex Trajectories - Time Step $(time_step) - γ=$(DISCOUNT_FACTOR), Events: $(count(==(EVENT_PRESENT), environment_state)) | Agents: $(length(agents)) | Detected: $(events_detected) | Avg Uncertainty: $(round(avg_uncertainty, digits=3))",
        titlefontsize=12,
...
function simulate_rsp_async_planning_replay(replay_env::ReplayEnvironment, num_steps::Int=NUM_STEPS, run_number::Int=1, planning_mode::Symbol=:script)
    ...
    # Update environment with current ground truth for oracle planner
    ...
    for t in 0:(num_steps-1)
        ...
        for agent in agents
            ...
            if !isempty(action.target_cells)
                ...
                # Debug: print what was observed vs ground truth
                events_found = count(==(EVENT_PRESENT), observation.event_states)
                
                # For oracle, compare ground truth vs observation
                if plan_type == :oracle && !isempty(action.target_cells)
                    for (i, cell) in enumerate(action.target_cells)
                        x, y = cell
                        gt_state = current_environment[y, x]
                        obs_state = observation.event_states[i]
                        gt_name = (gt_state == EVENT_PRESENT) ? "EVENT" : "EMPTY"
                        obs_name = (obs_state == EVENT_PRESENT) ? "EVENT" : "EMPTY"
                        println("  🔍 Cell $(cell): ground_truth=$(gt_name), observed=$(obs_name)")
                        if gt_state != obs_state
                            println("    ⚠️  MISMATCH! Oracle saw $(gt_name) but sensor detected $(obs_name)")
                        end
                    end
                end
