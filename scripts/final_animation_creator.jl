...
"""
Final animation creator - uses the same approach as main.jl
No precompilation issues, just like the main files.
"""
...
using Plots
using Dates
Plots.plotlyjs()  # This is the key - same as main.jl
...
"""
Create RSP animation with labels - using same approach as main.jl
"""
function create_rsp_animation_with_labels(
    ...
    for (step, env_state) in enumerate(env_evolution)
        ...
        # Create plot using same approach as main.jl
        p = plot(
...
function main(results_folder::String)
    println("🎬 Creating Video Animations with Labels (Final Version)")
    println("========================================================")
    ...
    println("\n✅ ANIMATION CREATED! Check the animations folder for the new GIF with labels.")
end

# Run it
if length(ARGS) != 1
    println("Usage: julia final_animation_creator.jl <results_folder>")
    exit(1)
end
