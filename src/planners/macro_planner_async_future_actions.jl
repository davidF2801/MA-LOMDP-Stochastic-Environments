module MacroPlannerAsyncFutureActions
        ...
        for (B, p_branch) in B_post[t_global]
            ...
            for missing_agent_id in missing_agents
                if missing_agent_id ∉ agents_with_sub_plans
                    ...
                    agent_sub_plan = generate_sub_plans(seq, C, env, B_branches, gs_state, t_start, t_end, missing_agent_id)
...
function generate_sub_plans(seq, C, env, B_branches, gs_state, t_start, t_end, missing_agent_id)
    ...
    for sequence in action_sequences
        ...
        value = evaluate_sub_sequence(sequence, env, agent, t_start, gs_state, B_branches)
        if value > best_value
            best_value = value
            best_sequence = sequence
        end
...
"""
function evaluate_sub_sequence(seq::Vector{SensingAction}, env, agent, t_start::Int, gs_state, B_branches)
    γ = env.discount
    c_obs = 0.0  # Cost of performing an observation
    
    # Use belief branches starting from t_start
    B_post = Dict{Int, Vector{Tuple{Belief, Float64}}}()
    B_post[t_start] = B_branches[t_start]
